...
$handler->onEndExportReport = function ($event) {
	...
	file_put_contents('stimulsoft/other/'.$fileName.'.'.strtolower($format), base64_decode($data));
...
$handler->onSaveReport = function ($event) {
    ...
    if(is_dir("customReports/")) {
        //return("$folderName is a directory");
        file_put_contents('customReports/'.$fileName.".mrt", $reportJson);
    }
    else {
        mkdir("customReports/");
        //file_put_contents('reports/'.$fileName.".mrt", $reportJson);
        file_put_contents('stimulsoft/customReports/'.$fileName.".mrt", $reportJson);
    }
