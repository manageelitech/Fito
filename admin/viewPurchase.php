...
$table.=  '<div class="box-body table-responsive no-padding"><table  class="table table-hover" id="orderttable">
                        ...
                        <tbody>';
$slno = 0;
$qry=mysqli_query($conn,"SELECT distinct orderNo FROM orders where userid='$uid' and status='confirm' and orderNo is not null");
while($res=mysqli_fetch_array($qry))
{
    ...
    $table.='
            <tr>
                            ...
                            <td class="">'.$date.'</td>';
    if ($utype == 'dist'){
        $table.='<td class=""><a href="billing.php?order='.$orderNo.'" target="_blank" style="color: #4c23e1;">VIEW INVOICE</a></td>';
    }
    else{
        $table.='<td class=""><a href="billg.php?order='.$orderNo.'" target="_blank" style="color: #4c23e1;">VIEW INVOICE</a></td>';
    }
    $table.='</tr>';
}
